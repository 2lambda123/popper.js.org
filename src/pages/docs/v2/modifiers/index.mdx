...
const topLogger = {
  ...
  phase: 'main',
  fn({ state }) {
    if (state.placement === 'top') {    console.log('Popper is on the top');
...
populated with custom options to override the options of a built-in
modifier.
...
that read them rather than querying the DOM, optimizing the overall
